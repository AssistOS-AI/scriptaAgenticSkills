@central-idea define
hook-pattern: what-if
tension-source: mixed
naming-state: placeholder
hook: Memory architect Elian Quill discovers that the orbital station Nacre keeps social harmony by erasing witness memory whenever dissent threatens the station council. With shuttle pilot Yara Sen she traces a series of benign technical anomalies back to a hidden archive where entire political decisions have been amputated from collective recall.
protagonist: Elian Quill
desire: trace the corrupted recall patterns and restore the missing vote archive
opposition: Director Cael Oris, station council chair and guardian of the memory pacification protocol, backed by Nacre Station Council
stakes: the station's political legitimacy, Elian's own complicity, and the residents' right to remember
dilemma: Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
story-question: Can Elian restore the station's memory without destroying the fragile peace built on forgetting?
audience-fit: science fiction readers
pitch-test: Elian Quill must trace the corrupted recall patterns and restore the missing vote archive while Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
A memory architect learns that her orbital station stays peaceful by deleting politically inconvenient memories.
The symbolic seed leaves surface naming unresolved on purpose so later refinement can introduce more natural language detail.

@theme define
topic: freedom-security
moral-shape: dilemma
thematic-question: Is peace still peace when consent depends on not remembering the price?
thematic-statement: Safety purchased through engineered forgetting is only a delayed form of coercion.
pole-a: stability-through-erasure
pole-b: freedom-through-memory
moral-pressure: Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
transformation-vector: move Elian Quill from stable systems prevent larger human harm toward a system that edits consent cannot claim legitimacy even if it remains efficient
dramatization: major choices and consequences must carry the theme instead of direct exposition

@wisdom define
wisdom-dominant-dimension: balanced
cognitive: Can Elian restore the station's memory without destroying the fragile peace built on forgetting?
emotional: accept that elegant systems can become moral anesthesia
moral: Safety purchased through engineered forgetting is only a delayed form of coercion.
reflexive: the reflexive dimension of insight this story offers
experiential: the experiential dimension of insight this story offers
perspective-mode: single-perspective
openness: open
anti-didactic-rule: insight must emerge from consequences, reversals, and conflicting voices

@narrative-structure define
information-order: chronological
causal-density: loose
macro-form: linear
beginning: establish Elian Quill inside the mnemonic orchard ring and the pressure around Nacre Station Council
inciting-incident: the shard contains voices naming a vote that station records say never occurred
middle: escalation, pressure, and relational strain expose the cost of trace the corrupted recall patterns and restore the missing vote archive
plot-point-1: Elian Quill commits to action instead of withdrawal
midpoint: the erased council vote archive reveals a hidden layer of the conflict
plot-point-2: Director Cael Oris turns pressure into direct threat
climax: Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
resolution: a new equilibrium forms after Elian Quill accepts irreversible cost
causal-rule: every-escalation-must-narrow-the-safe-options

@narrative-model select
adaptation-strength: hybridized
model-name: equilibrium-disruption-restoration
core-pattern: investigation-reveals-hidden-structure
fit-reason: the science fiction profile needs direct escalation with meaningful internal cost
transformation-logic: Elian Quill changes while the surrounding system reveals its hidden logic
genre-compatibility: Science Fiction
adaptation-note: the chosen model is bent toward short-form clarity and evidence-rich validation

@blueprint map
premise: Memory architect Elian Quill discovers that the orbital station Nacre keeps social harmony by erasing witness memory whenever dissent threatens the station council. With shuttle pilot Yara Sen she traces a series of benign technical anomalies back to a hidden archive where entire political decisions have been amputated from collective recall.
exposition: introduce Elian Quill, the mnemonic orchard ring, and the first unstable sign around the erased council vote archive
rising-action: pressure from Director Cael Oris and Nacre Station Council complicates every attempt to act
midpoint-shift: the erased council vote archive changes what the protagonist understands about the conflict
climax: Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
resolution: the core conflict resolves with lasting cost and a redefined relation to Nacre Station Council
emotional-layer: intrigue -> tension -> surprise -> dread -> bittersweet-release
stakes-ladder: private-to-public

@arc-book-main map
arc-axis: freedom-security under dilemma pressure
opening-state: system-appears-stable-but-is-leaking
escalation-pattern: each-escalation-narrows-safe-options
midpoint-recognition: apparent-problem-hides-larger-moral-structure
climactic-choice: Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
ending-state: survival-remains-but-innocence-does-not

@arc-protagonist-main map
entry-belief: stable systems prevent larger human harm control-prevents-loss
core-wound: earlier-abandonment-transformed-need-into-logistics
pressure-line: every-repair-exposes-the-politics-in-the-machine
turning-insight: peace without memory turns citizens into well-maintained evidence gaps visible-cost-is-preferable-to-stable-falsehood
exit-belief: a system that edits consent cannot claim legitimacy even if it remains efficient visible-cost-is-preferable-to-stable-falsehood

@arc-relationship-main map
pair: Elian Quill, Yara Sen
entry-dynamic: professional-alliance-with-unequal-risk
stress-pattern: shared-work-keeps-exposing-emotional-imbalance
repair-condition: repair-requires-naming-the-most-damaging-truth
exit-dynamic: scarred-trust-rebuilt-on-painful-clarity

@motif-primary define
motif: orchard leaves flickering with absent voices
symbolic-function: bind-inheritance-to-cost-and-memory
recurrence-rule: the motif should recur across opening, midpoint, and late consequence scenes with altered meaning

@central-idea refine
ref: hook:what-if -> Memory architect Elian Quill discovers that the orbital station Nacre keeps social harmony by erasing witness memory whenever dissent threatens the station council. With shuttle pilot Yara Sen she traces a series of benign technical anomalies back to a hidden archive where entire political decisions have been amputated from collective recall.
ref: character:protagonist-001 -> Elian Quill
ref: desire:protagonist -> trace the corrupted recall patterns and restore the missing vote archive
ref: opposition:primary -> Director Cael Oris, station council chair and guardian of the memory pacification protocol, backed by Nacre Station Council
ref: stakes:freedom-consent-memory-agency -> the station's political legitimacy, Elian's own complicity, and the residents' right to remember
ref: dilemma:central -> Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
ref: story-question:central -> Can Elian restore the station's memory without destroying the fragile peace built on forgetting?
ref: pitch:what-if -> Elian Quill must trace the corrupted recall patterns and restore the missing vote archive while Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: use concrete system language without losing emotional readability
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@theme refine
ref: character:protagonist-001 -> Elian Quill
ref: dilemma:central -> Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
ref: thematic-question:primary -> Is peace still peace when consent depends on not remembering the price?
ref: thematic-statement:primary -> Safety purchased through engineered forgetting is only a delayed form of coercion.
ref: pole-a:primary -> stability-through-erasure
ref: pole-b:primary -> freedom-through-memory
ref: entry-belief:protagonist -> stable systems prevent larger human harm
ref: exit-belief:protagonist -> a system that edits consent cannot claim legitimacy even if it remains efficient
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: use concrete system language without losing emotional readability
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@arc-book-main refine
ref: dilemma:central -> Preserve a stable station that survives by selective erasure, or return memory to the people who may tear it apart once they remember what was done to them.
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make the arc bind system insight to ethical choice
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@arc-protagonist-main refine
ref: entry-belief:protagonist -> stable systems prevent larger human harm
ref: exit-belief:protagonist -> a system that edits consent cannot claim legitimacy even if it remains efficient
ref: turning-insight:protagonist -> peace without memory turns citizens into well-maintained evidence gaps
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make the arc bind system insight to ethical choice
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@arc-relationship-main refine
ref: character:protagonist-001 -> Elian Quill
ref: character:counterpart-001 -> Yara Sen
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make the arc bind system insight to ethical choice
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.
