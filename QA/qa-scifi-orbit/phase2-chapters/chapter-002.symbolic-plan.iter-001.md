@chapter-002 define
chapter-role: escalation
purpose: {{purpose:escalation}}
input-state: {{input-state:escalation-1}}
output-state: {{output-state:escalation-1}}
conflict: {{conflict:escalation}}
stakes: {{stakes:freedom-consent-memory-agency}}
opening-mode: aftermath-entry
development-mode: revelation-ladder
closing-mode: cliffhanger
handoff-pressure: the next chapter must open from the sharper consequence exposed in chapter-002
thematic-focus: freedom-security under dilemma pressure
rhythm-note: escalating
chapter-question: {{chapter-question:escalation-1}}
answer-shift: {{answer-shift:science-fiction-escalation-1}}
arc-stage: pressure-rise
world-pressure: {{world-pressure:escalation-1}}
location-focus: $location-primary
block-alternation: description-dialogue-action-interior-monologue-suspense-revelation-acceleration
This chapter must remain traceable to the symbolic seed so validation can compare chapter purpose to later prose.
