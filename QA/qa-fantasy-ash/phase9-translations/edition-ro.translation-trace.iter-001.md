# ro translation trace

Sequential chunk translation trace for the translated reader edition.

- book-id: qa-fantasy-ash
- source-language: en
- target-language: ro

## front.subtitle — chunk 1
- Source: Can Neris free the valley from her family's vow without leaving it defenseless against the cost the vow once contained?
- Translated: Poate Neris sa elibereze valea de juramantul familiei sale fara sa o lase fara aparare in fata pretului pe care juramantul il tinea candva inchis?

## front.premise — chunk 1
- Source: Exiled heir Neris Vale returns to the ash valley of Drovna when the family oath-fires begin speaking the names of the dead aloud.
- Translated: Exiled heir Neris Vale returns to the ash valley of Drovna when the family oath-fires begin speaking the names of the dead aloud.

## front.premise — chunk 2
- Source: Guided by oath-keeper Brother Caelan, she learns that her dynasty's power comes from a vow furnace that burns witness memories into political obedience
- Translated: Guided by oath-keeper Brother Caelan, she learns that her dynasty's power comes from a vow furnace that burns witness memories into political obedience

## front.premise — chunk 3
- Source: and breaking it will strip the valley of the same protection that kept the winter spirits out.
- Translated: and breaking it will strip the valley of the same protection that kept the winter spirits out.

## front.thematic-statement — chunk 1
- Source: Redemption begins when inherited power is treated as a debt to be relinquished rather than a right to be managed.
- Translated: Redemption begins when inherited power is treated as a debt to be relinquished rather than a right to be managed.

## front.world-rule — chunk 1
- Source: The valley wards hold only while an heir keeps the furnace vow active through lineage-bound ritual obedience.
- Translated: The valley wards hold only while an heir keeps the furnace vow active through lineage-bound ritual obedience.

## chapter.chapter-001.paragraph.1 — chunk 1
- Source: Neris Vale reaches the ash terrace above Drovna village, where ash, oath-smoke, and ancestral pressure cling to every threshold.
- Translated: Neris Vale ajunge in terasa de cenusa deasupra satului Drovna, unde cenusa, fumul juramantului si presiunea mostenirii se lipesc de fiecare prag.

## chapter.chapter-001.paragraph.2 — chunk 1
- Source: Neris returns to the terrace as the oath-fires begin whispering the names of children no one admits were taken.
- Translated: Neris returns to the terrace as the oath-fires begin whispering the names of children no one admits were taken.

## chapter.chapter-001.paragraph.3 — chunk 1
- Source: Later, brother caelan shows her a chronicle leaf spared from the furnace generations earlier.
- Translated: Later, brother caelan shows her a chronicle leaf spared from the furnace generations earlier.

## chapter.chapter-001.paragraph.4 — chunk 1
- Source: Later, the ash names neris's younger sister, long said to have died of fever.
- Translated: Later, the ash names neris's younger sister, long said to have died of fever.

## chapter.chapter-001.paragraph.5 — chunk 1
- Source: Nothing that has opened here will close easily.
- Translated: Nimic din ceea ce s-a deschis aici nu se va inchide usor.

## chapter.chapter-002.paragraph.1 — chunk 1
- Source: Neris Vale reaches the ash terrace above Drovna village, where ash, oath-smoke, and ancestral pressure cling to every threshold.
- Translated: Neris Vale ajunge in terasa de cenusa deasupra satului Drovna, unde cenusa, fumul juramantului si presiunea mostenirii se lipesc de fiecare prag.

## chapter.chapter-002.paragraph.2 — chunk 1
- Source: In the catacombs, Caelan reveals furnace records listing each life traded into the mountain wards.
- Translated: In the catacombs, caelan reveals furnace records listing each life traded into the mountain wards.

## chapter.chapter-002.paragraph.3 — chunk 1
- Source: Later, the records prove the vow consumed memory as well as blood, leaving entire households obedient but incomplete.
- Translated: Later, the records prove the vow consumed memory as well as blood, leaving entire households obedient but incomplete.

## chapter.chapter-002.paragraph.4 — chunk 1
- Source: Later, the records show neris's mother volunteered the sister's memory to preserve the wards.
- Translated: Later, Insemnarile arata ca mama lui Neris a oferit memoria surorii pentru a pastra ward-urile.

## chapter.chapter-002.paragraph.5 — chunk 1
- Source: Nothing that has opened here will close easily.
- Translated: Nimic din ceea ce s-a deschis aici nu se va inchide usor.

## chapter.chapter-003.paragraph.1 — chunk 1
- Source: Neris Vale reaches the ash terrace above Drovna village, where ash, oath-smoke, and ancestral pressure cling to every threshold.
- Translated: Neris Vale ajunge in terasa de cenusa deasupra satului Drovna, unde cenusa, fumul juramantului si presiunea mostenirii se lipesc de fiecare prag.

## chapter.chapter-003.paragraph.2 — chunk 1
- Source: A winter spirit tests the valley ward while Maeron accuses Neris of inviting the breach through disobedience.
- Translated: A winter spirit tests the valley ward while Maeron accuses Neris of inviting the breach through disobedience.

## chapter.chapter-003.paragraph.3 — chunk 1
- Source: Later, neris realizes the ward responds more strongly to named truth than to repeated liturgy.
- Translated: Later, neris realizes the ward responds more strongly to named truth than to repeated liturgy.

## chapter.chapter-003.paragraph.4 — chunk 1
- Source: Later, the winter spirit retreats only when neris names one of the forgotten dead aloud.
- Translated: Later, Spiritul iernii se retrage doar cand Neris rosteste cu voce tare numele unuia dintre mortii uitati.

## chapter.chapter-003.paragraph.5 — chunk 1
- Source: Nothing that has opened here will close easily.
- Translated: Nimic din ceea ce s-a deschis aici nu se va inchide usor.

## chapter.chapter-004.paragraph.1 — chunk 1
- Source: Neris Vale reaches the ash terrace above Drovna village, where ash, oath-smoke, and ancestral pressure cling to every threshold.
- Translated: Neris Vale ajunge in terasa de cenusa deasupra satului Drovna, unde cenusa, fumul juramantului si presiunea mostenirii se lipesc de fiecare prag.

## chapter.chapter-004.paragraph.2 — chunk 1
- Source: During the valley rite, Neris interrupts Maeron's succession oath with the stolen seal and the hidden chronicle.
- Translated: During the valley rite, Neris interrupts Maeron's succession oath with the stolen seal and the hidden chronicle.

## chapter.chapter-004.paragraph.3 — chunk 1
- Source: Later, she names the dead, breaks the seal, and forces the furnace to release the stored memories as ash-light across the terrace.
- Translated: Later, she names the dead, breaks the seal, and forces the furnace to release the stored memories as ash-light across the terrace.

## chapter.chapter-004.paragraph.4 — chunk 1
- Source: Later, the furnace spills the trapped names in a voice all of drovna can hear.
- Translated: Later, Cuptorul varsa numele captive intr-o voce pe care toata Drovna o poate auzi.

## chapter.chapter-004.paragraph.5 — chunk 1
- Source: Neris Vale understands now that some inheritances become just only when they are surrendered shared-risk-creates-stronger-agency.
- Translated: Neris Vale intelege acum ca some inheritances become just only when they are surrendered shared-risk-creates-stronger-agency.

## chapter.chapter-001.paragraph.1 — chunk 2
- Source: Every greeting sounds half like welcome and half like ritual surveillance.
- Translated: Fiecare salut suna pe jumatate a bun-venit si pe jumatate a supraveghere ritualica.

## chapter.chapter-001.paragraph.2 — chunk 2
- Source: Brother Caelan shows her a chronicle leaf spared from the furnace generations earlier.
- Translated: Brother Caelan shows her a chronicle leaf spared from the furnace generations earlier.

## chapter.chapter-001.paragraph.3 — chunk 2
- Source: Neris wants proof the whispers are more than grief-magic, while Caelan insists the valley is already warning her directly.
- Translated: Neris vrea dovezi ca soaptele sunt mai mult decat magie a doliului, in timp ce Caelan insista ca valea o avertizeaza deja direct.

## chapter.chapter-001.paragraph.4 — chunk 2
- Source: Family legend breaks open into accusation.
- Translated: Family legend breaks open into accusation.

## chapter.chapter-001.paragraph.5 — chunk 2
- Source: Can Neris free the valley from her family's vow without leaving it defenseless against the cost the vow once contained?.
- Translated: Poate Neris sa elibereze valea de juramantul familiei sale fara sa o lase fara aparare in fata pretului pe care juramantul il tinea candva inchis?.

## chapter.chapter-002.paragraph.1 — chunk 2
- Source: Every greeting sounds half like welcome and half like ritual surveillance.
- Translated: Fiecare salut suna pe jumatate a bun-venit si pe jumatate a supraveghere ritualica.

## chapter.chapter-002.paragraph.2 — chunk 2
- Source: The records prove the vow consumed memory as well as blood, leaving entire households obedient but incomplete.
- Translated: The records prove the vow consumed memory as well as blood, leaving entire households obedient but incomplete.

## chapter.chapter-002.paragraph.3 — chunk 2
- Source: Neris wants to expose Maeron immediately, yet doing so before the warding rite could leave the valley open to the winter breach.
- Translated: Neris vrea sa-l demaste imediat pe Maeron, dar daca face asta inainte de ritualul de aparare ar putea lasa valea deschisa spre ruptura iernii.

## chapter.chapter-002.paragraph.4 — chunk 2
- Source: Inheritance becomes personal wound rather than abstract lineage guilt.
- Translated: Inheritance becomes personal wound rather than abstract lineage guilt.

## chapter.chapter-003.paragraph.2 — chunk 2
- Source: Neris realizes the ward responds more strongly to named truth than to repeated liturgy.
- Translated: Neris realizes the ward responds more strongly to named truth than to repeated liturgy.

## chapter.chapter-002.paragraph.5 — chunk 2
- Source: Can Neris free the valley from her family's vow without leaving it defenseless against the cost the vow once contained?.
- Translated: Poate Neris sa elibereze valea de juramantul familiei sale fara sa o lase fara aparare in fata pretului pe care juramantul il tinea candva inchis?.

## chapter.chapter-003.paragraph.1 — chunk 2
- Source: Every greeting sounds half like welcome and half like ritual surveillance.
- Translated: Fiecare salut suna pe jumatate a bun-venit si pe jumatate a supraveghere ritualica.

## chapter.chapter-003.paragraph.3 — chunk 2
- Source: If she reveals the rite publicly, panic may shatter the valley before she can reforge the protection honestly.
- Translated: Daca dezvaluie public ritualul, panica ar putea sfarama valea inainte ca ea sa poata reface cinstit protectia.

## chapter.chapter-003.paragraph.4 — chunk 2
- Source: Truth reveals itself as the missing element in the valley's defense.
- Translated: Truth reveals itself as the missing element in the valley's defense.

## chapter.chapter-003.paragraph.5 — chunk 2
- Source: Can Neris free the valley from her family's vow without leaving it defenseless against the cost the vow once contained?.
- Translated: Poate Neris sa elibereze valea de juramantul familiei sale fara sa o lase fara aparare in fata pretului pe care juramantul il tinea candva inchis?.

## chapter.chapter-004.paragraph.1 — chunk 2
- Source: Every greeting sounds half like welcome and half like ritual surveillance.
- Translated: Fiecare salut suna pe jumatate a bun-venit si pe jumatate a supraveghere ritualica.

## chapter.chapter-004.paragraph.2 — chunk 2
- Source: She names the dead, breaks the seal, and forces the furnace to release the stored memories as ash-light across the terrace.
- Translated: She names the dead, breaks the seal, and forces the furnace to release the stored memories as ash-light across the terrace.

## chapter.chapter-004.paragraph.3 — chunk 2
- Source: The valley must choose whether to stand in fear with Maeron or bear the truth and rebuild the wards differently.
- Translated: Valea trebuie sa aleaga daca ramane in frica alaturi de Maeron sau daca suporta adevarul si recladeste ward-urile altfel.

## chapter.chapter-004.paragraph.4 — chunk 2
- Source: The valley witnesses the true cost of its protection at once.
- Translated: The valley witnesses the true cost of its protection at once.

## chapter.chapter-004.paragraph.5 — chunk 2
- Source: Altered-but-legible-bond.
- Translated: Altered-but-legible-bond.

## chapter.chapter-001.paragraph.2 — chunk 3
- Source: Neris wants proof the whispers are more than grief-magic, while Caelan insists the valley is already warning her directly.
- Translated: Neris vrea dovezi ca soaptele sunt mai mult decat magie a doliului, in timp ce Caelan insista ca valea o avertizeaza deja direct.

## chapter.chapter-001.paragraph.1 — chunk 3
- Source: Brother Caelan Vey reads the room too carefully, while Regent Maeron Vale keeps alive the version of events other people would rather believe.
- Translated: Brother Caelan Vey citeste prea atent incaperea, iar Regent Maeron Vale mentine vie versiunea faptelor pe care ceilalti ar prefera sa o creada.

## chapter.chapter-001.paragraph.3 — chunk 3
- Source: When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation.
- Translated: When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation.

## chapter.chapter-001.paragraph.4 — chunk 3
- Source: Neris wants proof the whispers are more than grief-magic, while Caelan insists the valley is already warning her directly.
- Translated: Neris vrea dovezi ca soaptele sunt mai mult decat magie a doliului, in timp ce Caelan insista ca valea o avertizeaza deja direct.

## chapter.chapter-002.paragraph.1 — chunk 3
- Source: Brother Caelan Vey reads the room too carefully, while Regent Maeron Vale keeps alive the version of events other people would rather believe.
- Translated: Brother Caelan Vey citeste prea atent incaperea, iar Regent Maeron Vale mentine vie versiunea faptelor pe care ceilalti ar prefera sa o creada.

## chapter.chapter-002.paragraph.2 — chunk 3
- Source: Neris wants to expose Maeron immediately, yet doing so before the warding rite could leave the valley open to the winter breach.
- Translated: Neris vrea sa-l demaste imediat pe Maeron, dar daca face asta inainte de ritualul de aparare ar putea lasa valea deschisa spre ruptura iernii.

## chapter.chapter-002.paragraph.3 — chunk 3
- Source: When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt.
- Translated: When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt.

## chapter.chapter-002.paragraph.4 — chunk 3
- Source: Neris wants to expose Maeron immediately, yet doing so before the warding rite could leave the valley open to the winter breach.
- Translated: Neris vrea sa-l demaste imediat pe Maeron, dar daca face asta inainte de ritualul de aparare ar putea lasa valea deschisa spre ruptura iernii.

## chapter.chapter-003.paragraph.2 — chunk 3
- Source: If she reveals the rite publicly, panic may shatter the valley before she can reforge the protection honestly.
- Translated: Daca dezvaluie public ritualul, panica ar putea sfarama valea inainte ca ea sa poata reface cinstit protectia.

## chapter.chapter-003.paragraph.1 — chunk 3
- Source: Brother Caelan Vey reads the room too carefully, while Regent Maeron Vale keeps alive the version of events other people would rather believe.
- Translated: Brother Caelan Vey citeste prea atent incaperea, iar Regent Maeron Vale mentine vie versiunea faptelor pe care ceilalti ar prefera sa o creada.

## chapter.chapter-003.paragraph.3 — chunk 3
- Source: When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense.
- Translated: When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense.

## chapter.chapter-003.paragraph.4 — chunk 3
- Source: If she reveals the rite publicly, panic may shatter the valley before she can reforge the protection honestly.
- Translated: Daca dezvaluie public ritualul, panica ar putea sfarama valea inainte ca ea sa poata reface cinstit protectia.

## chapter.chapter-004.paragraph.1 — chunk 3
- Source: Brother Caelan Vey reads the room too carefully, while Regent Maeron Vale keeps alive the version of events other people would rather believe.
- Translated: Brother Caelan Vey citeste prea atent incaperea, iar Regent Maeron Vale mentine vie versiunea faptelor pe care ceilalti ar prefera sa o creada.

## chapter.chapter-004.paragraph.2 — chunk 3
- Source: The valley must choose whether to stand in fear with Maeron or bear the truth and rebuild the wards differently.
- Translated: Valea trebuie sa aleaga daca ramane in frica alaturi de Maeron sau daca suporta adevarul si recladeste ward-urile altfel.

## chapter.chapter-004.paragraph.3 — chunk 3
- Source: When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once.
- Translated: When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once.

## chapter.chapter-004.paragraph.4 — chunk 3
- Source: The valley must choose whether to stand in fear with Maeron or bear the truth and rebuild the wards differently.
- Translated: Valea trebuie sa aleaga daca ramane in frica alaturi de Maeron sau daca suporta adevarul si recladeste ward-urile altfel.

## chapter.chapter-001.paragraph.2 — chunk 4
- Source: When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation.
- Translated: When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation.

## chapter.chapter-001.paragraph.3 — chunk 4
- Source: They descend toward the catacombs before Maeron can bind her publicly at dusk.
- Translated: They descend toward the catacombs before Maeron can bind her publicly at dusk.

## chapter.chapter-001.paragraph.4 — chunk 4
- Source: When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation.
- Translated: When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation.

## chapter.chapter-002.paragraph.2 — chunk 4
- Source: When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt.
- Translated: When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt.

## chapter.chapter-002.paragraph.3 — chunk 4
- Source: She steals the regent's binding seal instead, buying time but alerting him to the betrayal.
- Translated: She steals the regent's binding seal instead, buying time but alerting him to the betrayal.

## chapter.chapter-002.paragraph.4 — chunk 4
- Source: When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt.
- Translated: When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt.

## chapter.chapter-003.paragraph.2 — chunk 4
- Source: When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense.
- Translated: When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense.

## chapter.chapter-003.paragraph.3 — chunk 4
- Source: She decides to break the binding ceremony and speak the hidden names during the next warding chant.
- Translated: She decides to break the binding ceremony and speak the hidden names during the next warding chant.

## chapter.chapter-003.paragraph.4 — chunk 4
- Source: When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense.
- Translated: When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense.

## chapter.chapter-004.paragraph.2 — chunk 4
- Source: When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once.
- Translated: When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once.

## chapter.chapter-004.paragraph.3 — chunk 4
- Source: Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones.
- Translated: Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones.

## chapter.chapter-004.paragraph.4 — chunk 4
- Source: When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once.
- Translated: When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once.

## chapter.chapter-001.paragraph.2 — chunk 5
- Source: They descend toward the catacombs before Maeron can bind her publicly at dusk.
- Translated: They descend toward the catacombs before Maeron can bind her publicly at dusk.

## chapter.chapter-001.paragraph.4 — chunk 5
- Source: They descend toward the catacombs before Maeron can bind her publicly at dusk.
- Translated: They descend toward the catacombs before Maeron can bind her publicly at dusk.

## chapter.chapter-002.paragraph.2 — chunk 5
- Source: She steals the regent's binding seal instead, buying time but alerting him to the betrayal.
- Translated: She steals the regent's binding seal instead, buying time but alerting him to the betrayal.

## chapter.chapter-002.paragraph.4 — chunk 5
- Source: She steals the regent's binding seal instead, buying time but alerting him to the betrayal.
- Translated: She steals the regent's binding seal instead, buying time but alerting him to the betrayal.

## chapter.chapter-003.paragraph.2 — chunk 5
- Source: She decides to break the binding ceremony and speak the hidden names during the next warding chant.
- Translated: She decides to break the binding ceremony and speak the hidden names during the next warding chant.

## chapter.chapter-003.paragraph.4 — chunk 5
- Source: She decides to break the binding ceremony and speak the hidden names during the next warding chant.
- Translated: She decides to break the binding ceremony and speak the hidden names during the next warding chant.

## chapter.chapter-004.paragraph.2 — chunk 5
- Source: Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones.
- Translated: Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones.

## chapter.chapter-004.paragraph.4 — chunk 5
- Source: Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones.
- Translated: Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones.

<!-- scripta-data
{
  "bookId": "qa-fantasy-ash",
  "sourceLanguage": "en",
  "targetLanguage": "ro",
  "chunkCount": 82,
  "chunks": [
    {
      "path": "front.subtitle",
      "chunkIndex": 1,
      "source": "Can Neris free the valley from her family's vow without leaving it defenseless against the cost the vow once contained?",
      "translated": "Poate Neris sa elibereze valea de juramantul familiei sale fara sa o lase fara aparare in fata pretului pe care juramantul il tinea candva inchis?"
    },
    {
      "path": "front.premise",
      "chunkIndex": 1,
      "source": "Exiled heir Neris Vale returns to the ash valley of Drovna when the family oath-fires begin speaking the names of the dead aloud.",
      "translated": "Exiled heir Neris Vale returns to the ash valley of Drovna when the family oath-fires begin speaking the names of the dead aloud."
    },
    {
      "path": "front.premise",
      "chunkIndex": 2,
      "source": "Guided by oath-keeper Brother Caelan, she learns that her dynasty's power comes from a vow furnace that burns witness memories into political obedience",
      "translated": "Guided by oath-keeper Brother Caelan, she learns that her dynasty's power comes from a vow furnace that burns witness memories into political obedience"
    },
    {
      "path": "front.premise",
      "chunkIndex": 3,
      "source": "and breaking it will strip the valley of the same protection that kept the winter spirits out.",
      "translated": "and breaking it will strip the valley of the same protection that kept the winter spirits out."
    },
    {
      "path": "front.thematic-statement",
      "chunkIndex": 1,
      "source": "Redemption begins when inherited power is treated as a debt to be relinquished rather than a right to be managed.",
      "translated": "Redemption begins when inherited power is treated as a debt to be relinquished rather than a right to be managed."
    },
    {
      "path": "front.world-rule",
      "chunkIndex": 1,
      "source": "The valley wards hold only while an heir keeps the furnace vow active through lineage-bound ritual obedience.",
      "translated": "The valley wards hold only while an heir keeps the furnace vow active through lineage-bound ritual obedience."
    },
    {
      "path": "chapter.chapter-001.paragraph.1",
      "chunkIndex": 1,
      "source": "Neris Vale reaches the ash terrace above Drovna village, where ash, oath-smoke, and ancestral pressure cling to every threshold.",
      "translated": "Neris Vale ajunge in terasa de cenusa deasupra satului Drovna, unde cenusa, fumul juramantului si presiunea mostenirii se lipesc de fiecare prag."
    },
    {
      "path": "chapter.chapter-001.paragraph.2",
      "chunkIndex": 1,
      "source": "Neris returns to the terrace as the oath-fires begin whispering the names of children no one admits were taken.",
      "translated": "Neris returns to the terrace as the oath-fires begin whispering the names of children no one admits were taken."
    },
    {
      "path": "chapter.chapter-001.paragraph.3",
      "chunkIndex": 1,
      "source": "Later, brother caelan shows her a chronicle leaf spared from the furnace generations earlier.",
      "translated": "Later, brother caelan shows her a chronicle leaf spared from the furnace generations earlier."
    },
    {
      "path": "chapter.chapter-001.paragraph.4",
      "chunkIndex": 1,
      "source": "Later, the ash names neris's younger sister, long said to have died of fever.",
      "translated": "Later, the ash names neris's younger sister, long said to have died of fever."
    },
    {
      "path": "chapter.chapter-001.paragraph.5",
      "chunkIndex": 1,
      "source": "Nothing that has opened here will close easily.",
      "translated": "Nimic din ceea ce s-a deschis aici nu se va inchide usor."
    },
    {
      "path": "chapter.chapter-002.paragraph.1",
      "chunkIndex": 1,
      "source": "Neris Vale reaches the ash terrace above Drovna village, where ash, oath-smoke, and ancestral pressure cling to every threshold.",
      "translated": "Neris Vale ajunge in terasa de cenusa deasupra satului Drovna, unde cenusa, fumul juramantului si presiunea mostenirii se lipesc de fiecare prag."
    },
    {
      "path": "chapter.chapter-002.paragraph.2",
      "chunkIndex": 1,
      "source": "In the catacombs, Caelan reveals furnace records listing each life traded into the mountain wards.",
      "translated": "In the catacombs, caelan reveals furnace records listing each life traded into the mountain wards."
    },
    {
      "path": "chapter.chapter-002.paragraph.3",
      "chunkIndex": 1,
      "source": "Later, the records prove the vow consumed memory as well as blood, leaving entire households obedient but incomplete.",
      "translated": "Later, the records prove the vow consumed memory as well as blood, leaving entire households obedient but incomplete."
    },
    {
      "path": "chapter.chapter-002.paragraph.4",
      "chunkIndex": 1,
      "source": "Later, the records show neris's mother volunteered the sister's memory to preserve the wards.",
      "translated": "Later, Insemnarile arata ca mama lui Neris a oferit memoria surorii pentru a pastra ward-urile."
    },
    {
      "path": "chapter.chapter-002.paragraph.5",
      "chunkIndex": 1,
      "source": "Nothing that has opened here will close easily.",
      "translated": "Nimic din ceea ce s-a deschis aici nu se va inchide usor."
    },
    {
      "path": "chapter.chapter-003.paragraph.1",
      "chunkIndex": 1,
      "source": "Neris Vale reaches the ash terrace above Drovna village, where ash, oath-smoke, and ancestral pressure cling to every threshold.",
      "translated": "Neris Vale ajunge in terasa de cenusa deasupra satului Drovna, unde cenusa, fumul juramantului si presiunea mostenirii se lipesc de fiecare prag."
    },
    {
      "path": "chapter.chapter-003.paragraph.2",
      "chunkIndex": 1,
      "source": "A winter spirit tests the valley ward while Maeron accuses Neris of inviting the breach through disobedience.",
      "translated": "A winter spirit tests the valley ward while Maeron accuses Neris of inviting the breach through disobedience."
    },
    {
      "path": "chapter.chapter-003.paragraph.3",
      "chunkIndex": 1,
      "source": "Later, neris realizes the ward responds more strongly to named truth than to repeated liturgy.",
      "translated": "Later, neris realizes the ward responds more strongly to named truth than to repeated liturgy."
    },
    {
      "path": "chapter.chapter-003.paragraph.4",
      "chunkIndex": 1,
      "source": "Later, the winter spirit retreats only when neris names one of the forgotten dead aloud.",
      "translated": "Later, Spiritul iernii se retrage doar cand Neris rosteste cu voce tare numele unuia dintre mortii uitati."
    },
    {
      "path": "chapter.chapter-003.paragraph.5",
      "chunkIndex": 1,
      "source": "Nothing that has opened here will close easily.",
      "translated": "Nimic din ceea ce s-a deschis aici nu se va inchide usor."
    },
    {
      "path": "chapter.chapter-004.paragraph.1",
      "chunkIndex": 1,
      "source": "Neris Vale reaches the ash terrace above Drovna village, where ash, oath-smoke, and ancestral pressure cling to every threshold.",
      "translated": "Neris Vale ajunge in terasa de cenusa deasupra satului Drovna, unde cenusa, fumul juramantului si presiunea mostenirii se lipesc de fiecare prag."
    },
    {
      "path": "chapter.chapter-004.paragraph.2",
      "chunkIndex": 1,
      "source": "During the valley rite, Neris interrupts Maeron's succession oath with the stolen seal and the hidden chronicle.",
      "translated": "During the valley rite, Neris interrupts Maeron's succession oath with the stolen seal and the hidden chronicle."
    },
    {
      "path": "chapter.chapter-004.paragraph.3",
      "chunkIndex": 1,
      "source": "Later, she names the dead, breaks the seal, and forces the furnace to release the stored memories as ash-light across the terrace.",
      "translated": "Later, she names the dead, breaks the seal, and forces the furnace to release the stored memories as ash-light across the terrace."
    },
    {
      "path": "chapter.chapter-004.paragraph.4",
      "chunkIndex": 1,
      "source": "Later, the furnace spills the trapped names in a voice all of drovna can hear.",
      "translated": "Later, Cuptorul varsa numele captive intr-o voce pe care toata Drovna o poate auzi."
    },
    {
      "path": "chapter.chapter-004.paragraph.5",
      "chunkIndex": 1,
      "source": "Neris Vale understands now that some inheritances become just only when they are surrendered shared-risk-creates-stronger-agency.",
      "translated": "Neris Vale intelege acum ca some inheritances become just only when they are surrendered shared-risk-creates-stronger-agency."
    },
    {
      "path": "chapter.chapter-001.paragraph.1",
      "chunkIndex": 2,
      "source": "Every greeting sounds half like welcome and half like ritual surveillance.",
      "translated": "Fiecare salut suna pe jumatate a bun-venit si pe jumatate a supraveghere ritualica."
    },
    {
      "path": "chapter.chapter-001.paragraph.2",
      "chunkIndex": 2,
      "source": "Brother Caelan shows her a chronicle leaf spared from the furnace generations earlier.",
      "translated": "Brother Caelan shows her a chronicle leaf spared from the furnace generations earlier."
    },
    {
      "path": "chapter.chapter-001.paragraph.3",
      "chunkIndex": 2,
      "source": "Neris wants proof the whispers are more than grief-magic, while Caelan insists the valley is already warning her directly.",
      "translated": "Neris vrea dovezi ca soaptele sunt mai mult decat magie a doliului, in timp ce Caelan insista ca valea o avertizeaza deja direct."
    },
    {
      "path": "chapter.chapter-001.paragraph.4",
      "chunkIndex": 2,
      "source": "Family legend breaks open into accusation.",
      "translated": "Family legend breaks open into accusation."
    },
    {
      "path": "chapter.chapter-001.paragraph.5",
      "chunkIndex": 2,
      "source": "Can Neris free the valley from her family's vow without leaving it defenseless against the cost the vow once contained?.",
      "translated": "Poate Neris sa elibereze valea de juramantul familiei sale fara sa o lase fara aparare in fata pretului pe care juramantul il tinea candva inchis?."
    },
    {
      "path": "chapter.chapter-002.paragraph.1",
      "chunkIndex": 2,
      "source": "Every greeting sounds half like welcome and half like ritual surveillance.",
      "translated": "Fiecare salut suna pe jumatate a bun-venit si pe jumatate a supraveghere ritualica."
    },
    {
      "path": "chapter.chapter-002.paragraph.2",
      "chunkIndex": 2,
      "source": "The records prove the vow consumed memory as well as blood, leaving entire households obedient but incomplete.",
      "translated": "The records prove the vow consumed memory as well as blood, leaving entire households obedient but incomplete."
    },
    {
      "path": "chapter.chapter-002.paragraph.3",
      "chunkIndex": 2,
      "source": "Neris wants to expose Maeron immediately, yet doing so before the warding rite could leave the valley open to the winter breach.",
      "translated": "Neris vrea sa-l demaste imediat pe Maeron, dar daca face asta inainte de ritualul de aparare ar putea lasa valea deschisa spre ruptura iernii."
    },
    {
      "path": "chapter.chapter-002.paragraph.4",
      "chunkIndex": 2,
      "source": "Inheritance becomes personal wound rather than abstract lineage guilt.",
      "translated": "Inheritance becomes personal wound rather than abstract lineage guilt."
    },
    {
      "path": "chapter.chapter-002.paragraph.5",
      "chunkIndex": 2,
      "source": "Can Neris free the valley from her family's vow without leaving it defenseless against the cost the vow once contained?.",
      "translated": "Poate Neris sa elibereze valea de juramantul familiei sale fara sa o lase fara aparare in fata pretului pe care juramantul il tinea candva inchis?."
    },
    {
      "path": "chapter.chapter-003.paragraph.2",
      "chunkIndex": 2,
      "source": "Neris realizes the ward responds more strongly to named truth than to repeated liturgy.",
      "translated": "Neris realizes the ward responds more strongly to named truth than to repeated liturgy."
    },
    {
      "path": "chapter.chapter-003.paragraph.1",
      "chunkIndex": 2,
      "source": "Every greeting sounds half like welcome and half like ritual surveillance.",
      "translated": "Fiecare salut suna pe jumatate a bun-venit si pe jumatate a supraveghere ritualica."
    },
    {
      "path": "chapter.chapter-003.paragraph.3",
      "chunkIndex": 2,
      "source": "If she reveals the rite publicly, panic may shatter the valley before she can reforge the protection honestly.",
      "translated": "Daca dezvaluie public ritualul, panica ar putea sfarama valea inainte ca ea sa poata reface cinstit protectia."
    },
    {
      "path": "chapter.chapter-003.paragraph.4",
      "chunkIndex": 2,
      "source": "Truth reveals itself as the missing element in the valley's defense.",
      "translated": "Truth reveals itself as the missing element in the valley's defense."
    },
    {
      "path": "chapter.chapter-003.paragraph.5",
      "chunkIndex": 2,
      "source": "Can Neris free the valley from her family's vow without leaving it defenseless against the cost the vow once contained?.",
      "translated": "Poate Neris sa elibereze valea de juramantul familiei sale fara sa o lase fara aparare in fata pretului pe care juramantul il tinea candva inchis?."
    },
    {
      "path": "chapter.chapter-004.paragraph.1",
      "chunkIndex": 2,
      "source": "Every greeting sounds half like welcome and half like ritual surveillance.",
      "translated": "Fiecare salut suna pe jumatate a bun-venit si pe jumatate a supraveghere ritualica."
    },
    {
      "path": "chapter.chapter-004.paragraph.2",
      "chunkIndex": 2,
      "source": "She names the dead, breaks the seal, and forces the furnace to release the stored memories as ash-light across the terrace.",
      "translated": "She names the dead, breaks the seal, and forces the furnace to release the stored memories as ash-light across the terrace."
    },
    {
      "path": "chapter.chapter-004.paragraph.3",
      "chunkIndex": 2,
      "source": "The valley must choose whether to stand in fear with Maeron or bear the truth and rebuild the wards differently.",
      "translated": "Valea trebuie sa aleaga daca ramane in frica alaturi de Maeron sau daca suporta adevarul si recladeste ward-urile altfel."
    },
    {
      "path": "chapter.chapter-004.paragraph.4",
      "chunkIndex": 2,
      "source": "The valley witnesses the true cost of its protection at once.",
      "translated": "The valley witnesses the true cost of its protection at once."
    },
    {
      "path": "chapter.chapter-004.paragraph.5",
      "chunkIndex": 2,
      "source": "Altered-but-legible-bond.",
      "translated": "Altered-but-legible-bond."
    },
    {
      "path": "chapter.chapter-001.paragraph.2",
      "chunkIndex": 3,
      "source": "Neris wants proof the whispers are more than grief-magic, while Caelan insists the valley is already warning her directly.",
      "translated": "Neris vrea dovezi ca soaptele sunt mai mult decat magie a doliului, in timp ce Caelan insista ca valea o avertizeaza deja direct."
    },
    {
      "path": "chapter.chapter-001.paragraph.1",
      "chunkIndex": 3,
      "source": "Brother Caelan Vey reads the room too carefully, while Regent Maeron Vale keeps alive the version of events other people would rather believe.",
      "translated": "Brother Caelan Vey citeste prea atent incaperea, iar Regent Maeron Vale mentine vie versiunea faptelor pe care ceilalti ar prefera sa o creada."
    },
    {
      "path": "chapter.chapter-001.paragraph.3",
      "chunkIndex": 3,
      "source": "When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation.",
      "translated": "When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation."
    },
    {
      "path": "chapter.chapter-001.paragraph.4",
      "chunkIndex": 3,
      "source": "Neris wants proof the whispers are more than grief-magic, while Caelan insists the valley is already warning her directly.",
      "translated": "Neris vrea dovezi ca soaptele sunt mai mult decat magie a doliului, in timp ce Caelan insista ca valea o avertizeaza deja direct."
    },
    {
      "path": "chapter.chapter-002.paragraph.1",
      "chunkIndex": 3,
      "source": "Brother Caelan Vey reads the room too carefully, while Regent Maeron Vale keeps alive the version of events other people would rather believe.",
      "translated": "Brother Caelan Vey citeste prea atent incaperea, iar Regent Maeron Vale mentine vie versiunea faptelor pe care ceilalti ar prefera sa o creada."
    },
    {
      "path": "chapter.chapter-002.paragraph.2",
      "chunkIndex": 3,
      "source": "Neris wants to expose Maeron immediately, yet doing so before the warding rite could leave the valley open to the winter breach.",
      "translated": "Neris vrea sa-l demaste imediat pe Maeron, dar daca face asta inainte de ritualul de aparare ar putea lasa valea deschisa spre ruptura iernii."
    },
    {
      "path": "chapter.chapter-002.paragraph.3",
      "chunkIndex": 3,
      "source": "When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt.",
      "translated": "When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt."
    },
    {
      "path": "chapter.chapter-002.paragraph.4",
      "chunkIndex": 3,
      "source": "Neris wants to expose Maeron immediately, yet doing so before the warding rite could leave the valley open to the winter breach.",
      "translated": "Neris vrea sa-l demaste imediat pe Maeron, dar daca face asta inainte de ritualul de aparare ar putea lasa valea deschisa spre ruptura iernii."
    },
    {
      "path": "chapter.chapter-003.paragraph.2",
      "chunkIndex": 3,
      "source": "If she reveals the rite publicly, panic may shatter the valley before she can reforge the protection honestly.",
      "translated": "Daca dezvaluie public ritualul, panica ar putea sfarama valea inainte ca ea sa poata reface cinstit protectia."
    },
    {
      "path": "chapter.chapter-003.paragraph.1",
      "chunkIndex": 3,
      "source": "Brother Caelan Vey reads the room too carefully, while Regent Maeron Vale keeps alive the version of events other people would rather believe.",
      "translated": "Brother Caelan Vey citeste prea atent incaperea, iar Regent Maeron Vale mentine vie versiunea faptelor pe care ceilalti ar prefera sa o creada."
    },
    {
      "path": "chapter.chapter-003.paragraph.3",
      "chunkIndex": 3,
      "source": "When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense.",
      "translated": "When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense."
    },
    {
      "path": "chapter.chapter-003.paragraph.4",
      "chunkIndex": 3,
      "source": "If she reveals the rite publicly, panic may shatter the valley before she can reforge the protection honestly.",
      "translated": "Daca dezvaluie public ritualul, panica ar putea sfarama valea inainte ca ea sa poata reface cinstit protectia."
    },
    {
      "path": "chapter.chapter-004.paragraph.1",
      "chunkIndex": 3,
      "source": "Brother Caelan Vey reads the room too carefully, while Regent Maeron Vale keeps alive the version of events other people would rather believe.",
      "translated": "Brother Caelan Vey citeste prea atent incaperea, iar Regent Maeron Vale mentine vie versiunea faptelor pe care ceilalti ar prefera sa o creada."
    },
    {
      "path": "chapter.chapter-004.paragraph.2",
      "chunkIndex": 3,
      "source": "The valley must choose whether to stand in fear with Maeron or bear the truth and rebuild the wards differently.",
      "translated": "Valea trebuie sa aleaga daca ramane in frica alaturi de Maeron sau daca suporta adevarul si recladeste ward-urile altfel."
    },
    {
      "path": "chapter.chapter-004.paragraph.3",
      "chunkIndex": 3,
      "source": "When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once.",
      "translated": "When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once."
    },
    {
      "path": "chapter.chapter-004.paragraph.4",
      "chunkIndex": 3,
      "source": "The valley must choose whether to stand in fear with Maeron or bear the truth and rebuild the wards differently.",
      "translated": "Valea trebuie sa aleaga daca ramane in frica alaturi de Maeron sau daca suporta adevarul si recladeste ward-urile altfel."
    },
    {
      "path": "chapter.chapter-001.paragraph.2",
      "chunkIndex": 4,
      "source": "When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation.",
      "translated": "When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation."
    },
    {
      "path": "chapter.chapter-001.paragraph.3",
      "chunkIndex": 4,
      "source": "They descend toward the catacombs before Maeron can bind her publicly at dusk.",
      "translated": "They descend toward the catacombs before Maeron can bind her publicly at dusk."
    },
    {
      "path": "chapter.chapter-001.paragraph.4",
      "chunkIndex": 4,
      "source": "When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation.",
      "translated": "When the ash names Neris's younger sister, long said to have died of fever, family legend breaks open into accusation."
    },
    {
      "path": "chapter.chapter-002.paragraph.2",
      "chunkIndex": 4,
      "source": "When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt.",
      "translated": "When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt."
    },
    {
      "path": "chapter.chapter-002.paragraph.3",
      "chunkIndex": 4,
      "source": "She steals the regent's binding seal instead, buying time but alerting him to the betrayal.",
      "translated": "She steals the regent's binding seal instead, buying time but alerting him to the betrayal."
    },
    {
      "path": "chapter.chapter-002.paragraph.4",
      "chunkIndex": 4,
      "source": "When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt.",
      "translated": "When the records show Neris's mother volunteered the sister's memory to preserve the wards, inheritance becomes personal wound rather than abstract lineage guilt."
    },
    {
      "path": "chapter.chapter-003.paragraph.2",
      "chunkIndex": 4,
      "source": "When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense.",
      "translated": "When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense."
    },
    {
      "path": "chapter.chapter-003.paragraph.3",
      "chunkIndex": 4,
      "source": "She decides to break the binding ceremony and speak the hidden names during the next warding chant.",
      "translated": "She decides to break the binding ceremony and speak the hidden names during the next warding chant."
    },
    {
      "path": "chapter.chapter-003.paragraph.4",
      "chunkIndex": 4,
      "source": "When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense.",
      "translated": "When the winter spirit retreats only when Neris names one of the forgotten dead aloud, truth reveals itself as the missing element in the valley's defense."
    },
    {
      "path": "chapter.chapter-004.paragraph.2",
      "chunkIndex": 4,
      "source": "When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once.",
      "translated": "When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once."
    },
    {
      "path": "chapter.chapter-004.paragraph.3",
      "chunkIndex": 4,
      "source": "Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones.",
      "translated": "Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones."
    },
    {
      "path": "chapter.chapter-004.paragraph.4",
      "chunkIndex": 4,
      "source": "When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once.",
      "translated": "When the furnace spills the trapped names in a voice all of Drovna can hear, the valley witnesses the true cost of its protection at once."
    },
    {
      "path": "chapter.chapter-001.paragraph.2",
      "chunkIndex": 5,
      "source": "They descend toward the catacombs before Maeron can bind her publicly at dusk.",
      "translated": "They descend toward the catacombs before Maeron can bind her publicly at dusk."
    },
    {
      "path": "chapter.chapter-001.paragraph.4",
      "chunkIndex": 5,
      "source": "They descend toward the catacombs before Maeron can bind her publicly at dusk.",
      "translated": "They descend toward the catacombs before Maeron can bind her publicly at dusk."
    },
    {
      "path": "chapter.chapter-002.paragraph.2",
      "chunkIndex": 5,
      "source": "She steals the regent's binding seal instead, buying time but alerting him to the betrayal.",
      "translated": "She steals the regent's binding seal instead, buying time but alerting him to the betrayal."
    },
    {
      "path": "chapter.chapter-002.paragraph.4",
      "chunkIndex": 5,
      "source": "She steals the regent's binding seal instead, buying time but alerting him to the betrayal.",
      "translated": "She steals the regent's binding seal instead, buying time but alerting him to the betrayal."
    },
    {
      "path": "chapter.chapter-003.paragraph.2",
      "chunkIndex": 5,
      "source": "She decides to break the binding ceremony and speak the hidden names during the next warding chant.",
      "translated": "She decides to break the binding ceremony and speak the hidden names during the next warding chant."
    },
    {
      "path": "chapter.chapter-003.paragraph.4",
      "chunkIndex": 5,
      "source": "She decides to break the binding ceremony and speak the hidden names during the next warding chant.",
      "translated": "She decides to break the binding ceremony and speak the hidden names during the next warding chant."
    },
    {
      "path": "chapter.chapter-004.paragraph.2",
      "chunkIndex": 5,
      "source": "Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones.",
      "translated": "Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones."
    },
    {
      "path": "chapter.chapter-004.paragraph.4",
      "chunkIndex": 5,
      "source": "Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones.",
      "translated": "Maeron falls with the collapsing rite, and the valley joins Neris in a new chant built on remembered names rather than erased ones."
    }
  ]
}
-->
