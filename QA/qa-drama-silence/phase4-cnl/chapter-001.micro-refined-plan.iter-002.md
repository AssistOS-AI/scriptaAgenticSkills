@sequence-001-core define
sequence-type: recovery
link-logic: logical
chapter: chapter-001
objective: secure the recording and identify the file number mentioned in it
scene-chain: scene-001-01, scene-001-02, scene-001-03
continuity-thread: each scene must inherit and intensify the previous scene's unresolved pressure
conflict-line: Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
payoff: Mara can no longer pretend the death belongs only to the past

@location-001-anchor define
chapter: chapter-001
primary-setting: the shuttered mediation office above the old customs hall
secondary-setting: the flood memorial crypt beneath Saint Varin church
sensory-anchor: wet wool, radiator hiss, mildew trapped in paper bindings
social-signal: every conversation sounds temporary because the office trained people to trade truth for calm
symbolic-charge: a civic room designed to soften language until responsibility disappears
conflict-use: private negotiations keep collapsing into accusations that belong in public

@rule-pressure-001-core apply
chapter: chapter-001
rule-reference: world-rule-primary
visible-symptom: files return with fresh signatures but older dates, as if grief itself were rewritten overnight
action-limitation: Mara cannot accuse the town hall without exposing her mother and Iona to the same hearing.
conflict-output: bureaucratic delay turns each act of mourning into a negotiation over who gets to keep dignity
reveal-pattern: show-consequence-first-explain-cause-later

@arc-001-protagonist map
chapter: chapter-001
entry-belief: keeping a family safe matters more than forcing a public reckoning at the start of the setup chapter
challenge: Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
insight-pressure: the memorial becomes evidence instead of closure
exit-belief: care without truth only prolongs the violence of the original wound after the setup chapter

@arc-001-relationship map
chapter: chapter-001
pair: Mara Iliev, Iona Radu
entry-dynamic: professional-alliance-with-unequal-risk
stress-line: Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
exit-dynamic: altered-but-legible-bond

@alternation-001-core arrange
chapter: chapter-001
block-order: description-action-dialogue-pause-event-monologue
reader-effect: When grief becomes a civic ritual, is silence still mercy or only a softer form of power?
anti-flatness-rule: do not chain action summaries without dialogue, atmosphere, or reflection support

@scene-001-01 define
chapter: chapter-001
showing-mode: direct-showing
focalization: internal-single
time-space: the shuttered mediation office above the old customs hall
introduction: At her brother's memorial supper, Mara hears a hidden recording tucked inside a borrowed prayer book.
development: The message points toward one erased witness statement and a ledger that should not exist anymore.
conflict: Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
resolution: They agree to meet at dawn inside the sealed mediation office before the deputy mayor can collect the archive keys.
exit: the next scene begins before the pressure can settle
participants: Mara Iliev, Iona Radu, Lucian Petrescu
state-change: grief shifts from private mourning into a timed investigation

@action-001-1 place
action-mode: attempt
scene: scene-001-01
actor: Mara Iliev
goal: secure the recording and identify the file number mentioned in it
obstacle: family members keep interrupting every attempt to name what the voice implies
result: the attempt reveals only part of the hidden structure and deepens the next demand

@conflict-001-1 place
scope: scene-001-01
type: external-character
forces: Mara Iliev versus Lucian Petrescu
stakes: her brother's last testimony, her mother's survival, and the town's false innocence
escalation: The message points toward one erased witness statement and a ledger that should not exist anymore.

@event-001-1 trigger
scope: scene-001-01
event-type: discovery
trigger: the recording ends with Mara's brother naming Lucian before the tape cuts off
impact: the memorial becomes evidence instead of closure
follow-through: Mara can no longer pretend the death belongs only to the past

@dialogue-turn-001-01-01 line
scene: scene-001-01
speaker: Iona Radu
intent: tease-probe
subtext: Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
line-hint: the recording ends with Mara's brother naming Lucian before the tape cuts off
reaction-beat: how the other character reacts

@dialogue-turn-001-01-02 line
scene: scene-001-01
speaker: Mara Iliev
intent: tease-probe
subtext: Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
line-hint: the recording ends with Mara's brother naming Lucian before the tape cuts off
reaction-beat: how the other character reacts

@scene-001-02 define
chapter: chapter-001
showing-mode: compressed-showing
focalization: internal-single
time-space: the shuttered mediation office above the old customs hall
introduction: The message points toward one erased witness statement and a ledger that should not exist anymore.
development: Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
conflict: Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
resolution: They agree to meet at dawn inside the sealed mediation office before the deputy mayor can collect the archive keys.
exit: the next scene begins before the pressure can settle
participants: Mara Iliev, Iona Radu, Lucian Petrescu
state-change: grief shifts from private mourning into a timed investigation

@action-001-2 place
action-mode: attempt
scene: scene-001-02
actor: Mara Iliev
goal: Mara can no longer pretend the death belongs only to the past
obstacle: family members keep interrupting every attempt to name what the voice implies
result: the attempt reveals only part of the hidden structure and deepens the next demand

@conflict-001-2 place
scope: scene-001-02
type: external-nature
forces: Mara Iliev versus Lucian Petrescu
stakes: her brother's last testimony, her mother's survival, and the town's false innocence
escalation: The message points toward one erased witness statement and a ledger that should not exist anymore.

@event-001-2 trigger
scope: scene-001-02
event-type: crime
trigger: the recording ends with Mara's brother naming Lucian before the tape cuts off
impact: the memorial becomes evidence instead of closure
follow-through: Mara can no longer pretend the death belongs only to the past

@dialogue-turn-001-02-01 line
scene: scene-001-02
speaker: Iona Radu
intent: challenge
subtext: Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
line-hint: Memorial supper recording
reaction-beat: how the other character reacts

@dialogue-turn-001-02-02 line
scene: scene-001-02
speaker: Mara Iliev
intent: answer-honestly
subtext: Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
line-hint: Memorial supper recording
reaction-beat: how the other character reacts

@scene-001-03 define
chapter: chapter-001
showing-mode: compressed-showing
focalization: internal-single
time-space: the shuttered mediation office above the old customs hall
introduction: the recording ends with Mara's brother naming Lucian before the tape cuts off
development: the memorial becomes evidence instead of closure
conflict: Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
resolution: They agree to meet at dawn inside the sealed mediation office before the deputy mayor can collect the archive keys.
exit: the chapter hands off to a sharper setup consequence
participants: Mara Iliev, Iona Radu, Lucian Petrescu
state-change: grief shifts from private mourning into a timed investigation

@action-001-3 place
action-mode: negotiation
scene: scene-001-03
actor: Mara Iliev
goal: Memorial supper recording
obstacle: family members keep interrupting every attempt to name what the voice implies
result: grief shifts from private mourning into a timed investigation

@conflict-001-3 place
scope: scene-001-03
type: internal
forces: Mara Iliev versus Lucian Petrescu
stakes: her brother's last testimony, her mother's survival, and the town's false innocence
escalation: The message points toward one erased witness statement and a ledger that should not exist anymore.

@event-001-3 trigger
scope: scene-001-03
event-type: revelation
trigger: the recording ends with Mara's brother naming Lucian before the tape cuts off
impact: the memorial becomes evidence instead of closure
follow-through: Mara can no longer pretend the death belongs only to the past

@dialogue-turn-001-03-01 line
scene: scene-001-03
speaker: Iona Radu
intent: commit
subtext: Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
line-hint: The message points toward one erased witness statement and a ledger that should not exist anymore.
reaction-beat: how the other character reacts

@dialogue-turn-001-03-02 line
scene: scene-001-03
speaker: Mara Iliev
intent: warn
subtext: Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
line-hint: The message points toward one erased witness statement and a ledger that should not exist anymore.
reaction-beat: how the other character reacts

@description-001-atmosphere apply
scope: chapter-001
description-type: atmosphere
focus: a civic room designed to soften language until responsibility disappears
function: atmospheric
rhythm-effect: frame

@dialogue-001-core apply
scene: scene-001-01
speakers: Mara Iliev, Iona Radu
exchange-type: conflictual
purpose: tension
subtext: Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet

@narration-001-bridge apply
scope: chapter-001
narrator-mode: objective
function: organizational
time-handling: mixed

@interior-monologue-001-core apply
scene: scene-001-03
character: Mara Iliev
function: psychological-insight
trigger: the memorial becomes evidence instead of closure
texture: reflective

@suspense-001-core build
scope: chapter-001
suspense-type: emotional
uncertainty: Can Mara expose the lie that killed her brother if the truth also proves her family survived on the same silence?
delay-technique: mixed
payoff-zone: event-001-3
Delayed access, withheld explanation, and emotional pressure should work together instead of in isolation.

@pause-001-core hold
scope: chapter-001
pause-function: psychological
focus: grief shifts from private mourning into a timed investigation
placement: after-first-scene
reader-effect: decelerate just enough to let consequence become legible

@acceleration-001-core burst
scope: chapter-001
acceleration-mode: summary-burst
trigger: the recording ends with Mara's brother naming Lucian before the tape cuts off
reader-effect: compress time and force the next consequence to land without emotional escape
target-zone: scene-001-03

@cliffhanger-001-exit cut
scope: chapter-001
cliffhanger-type: interrupted-revelation
cut-moment: the recording ends with Mara's brother naming Lucian before the tape cuts off
continuation-pressure: Mara can no longer pretend the death belongs only to the past

@location-001-anchor refine
ref: location:primary-001 -> the shuttered mediation office above the old customs hall
ref: sensory-anchor:location-primary -> wet wool, radiator hiss, mildew trapped in paper bindings
ref: social-signal:location-primary -> every conversation sounds temporary because the office trained people to trade truth for calm
ref: symbolic-charge:location-primary -> a civic room designed to soften language until responsibility disappears
ref: conflict-use:location-primary -> private negotiations keep collapsing into accusations that belong in public
ref: location:secondary-001 -> the flood memorial crypt beneath Saint Varin church
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make the setting socially legible and emotionally pressurized instead of merely visual
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@rule-pressure-001-core refine
ref: visible-symptom:setup -> files return with fresh signatures but older dates, as if grief itself were rewritten overnight
ref: action-limitation:setup -> Mara cannot accuse the town hall without exposing her mother and Iona to the same hearing.
ref: conflict-output-rule:setup -> bureaucratic delay turns each act of mourning into a negotiation over who gets to keep dignity
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: show how institutions and norms shape action without over-explaining them
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@arc-001-protagonist refine
ref: entry-belief:protagonist -> keeping a family safe matters more than forcing a public reckoning
ref: exit-belief:protagonist -> care without truth only prolongs the violence of the original wound
ref: challenge:protagonist-setup -> Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
ref: insight-pressure:protagonist-setup -> the memorial becomes evidence instead of closure
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make the arc move through grief, blame, and difficult honesty
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@arc-001-relationship refine
ref: character:protagonist-001 -> Mara Iliev
ref: character:counterpart-001 -> Iona Radu
ref: relationship-stress:setup -> Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make the arc move through grief, blame, and difficult honesty
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@alternation-001-core refine
ref: reader-effect:setup -> When grief becomes a civic ritual, is silence still mercy or only a softer form of power?
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: let rhythm changes expose emotional consequence instead of merely changing speed
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@scene-001-01 refine
ref: character:protagonist-001 -> Mara Iliev
ref: location:primary-001 -> the shuttered mediation office above the old customs hall
ref: character:pressure-001 -> Lucian Petrescu
ref: character:counterpart-001 -> Iona Radu
ref: scene-introduction:setup-0 -> At her brother's memorial supper, Mara hears a hidden recording tucked inside a borrowed prayer book.
ref: scene-development:setup-0 -> The message points toward one erased witness statement and a ledger that should not exist anymore.
ref: scene-conflict:setup -> Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
ref: scene-resolution:setup-mid -> They agree to meet at dawn inside the sealed mediation office before the deputy mayor can collect the archive keys.
ref: scene-state-change:setup-mid -> grief shifts from private mourning into a timed investigation
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: let bodily tension and emotional hesitation appear before explanation
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@dialogue-turn-001-01-01 refine
ref: character:counterpart-001 -> Iona Radu
ref: dialogue-subtext:tease-probe -> Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
ref: dialogue-line-hint:setup-0-0 -> the recording ends with Mara's brother naming Lucian before the tape cuts off
ref: dialogue-reaction:tease-probe -> how the other character reacts
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make each line sound emotionally specific and morally contested rather than declarative
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@dialogue-turn-001-01-02 refine
ref: character:protagonist-001 -> Mara Iliev
ref: dialogue-subtext:tease-probe -> Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
ref: dialogue-reaction:tease-probe -> how the other character reacts
ref: dialogue-line-hint:setup-0-1 -> the recording ends with Mara's brother naming Lucian before the tape cuts off
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make each line sound emotionally specific and morally contested rather than declarative
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@scene-001-02 refine
ref: character:protagonist-001 -> Mara Iliev
ref: location:primary-001 -> the shuttered mediation office above the old customs hall
ref: character:pressure-001 -> Lucian Petrescu
ref: character:counterpart-001 -> Iona Radu
ref: scene-conflict:setup -> Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
ref: scene-resolution:setup-mid -> They agree to meet at dawn inside the sealed mediation office before the deputy mayor can collect the archive keys.
ref: scene-state-change:setup-mid -> grief shifts from private mourning into a timed investigation
ref: scene-introduction:setup-1 -> The message points toward one erased witness statement and a ledger that should not exist anymore.
ref: scene-development:setup-1 -> Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: let bodily tension and emotional hesitation appear before explanation
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@dialogue-turn-001-02-01 refine
ref: character:counterpart-001 -> Iona Radu
ref: dialogue-subtext:challenge -> Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
ref: dialogue-line-hint:setup-1-0 -> Memorial supper recording
ref: dialogue-reaction:challenge -> how the other character reacts
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make each line sound emotionally specific and morally contested rather than declarative
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@dialogue-turn-001-02-02 refine
ref: character:protagonist-001 -> Mara Iliev
ref: dialogue-subtext:answer-honestly -> Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
ref: dialogue-line-hint:setup-1-1 -> Memorial supper recording
ref: dialogue-reaction:answer-honestly -> how the other character reacts
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make each line sound emotionally specific and morally contested rather than declarative
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@scene-001-03 refine
ref: character:protagonist-001 -> Mara Iliev
ref: location:primary-001 -> the shuttered mediation office above the old customs hall
ref: character:pressure-001 -> Lucian Petrescu
ref: character:counterpart-001 -> Iona Radu
ref: scene-conflict:setup -> Mara needs Iona's help immediately, but Iona refuses to move without proof that will survive the town hall.
ref: scene-introduction:setup-2 -> the recording ends with Mara's brother naming Lucian before the tape cuts off
ref: scene-development:setup-2 -> the memorial becomes evidence instead of closure
ref: scene-resolution:setup-final -> They agree to meet at dawn inside the sealed mediation office before the deputy mayor can collect the archive keys.
ref: scene-state-change:setup-final -> grief shifts from private mourning into a timed investigation
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: let bodily tension and emotional hesitation appear before explanation
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@dialogue-turn-001-03-01 refine
ref: character:counterpart-001 -> Iona Radu
ref: dialogue-subtext:commit -> Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
ref: dialogue-line-hint:setup-2-0 -> The message points toward one erased witness statement and a ledger that should not exist anymore.
ref: dialogue-reaction:commit -> how the other character reacts
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make each line sound emotionally specific and morally contested rather than declarative
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@dialogue-turn-001-03-02 refine
ref: character:protagonist-001 -> Mara Iliev
ref: dialogue-subtext:warn -> Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
ref: dialogue-line-hint:setup-2-1 -> The message points toward one erased witness statement and a ledger that should not exist anymore.
ref: dialogue-reaction:warn -> how the other character reacts
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make each line sound emotionally specific and morally contested rather than declarative
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@dialogue-001-core refine
ref: character:protagonist-001 -> Mara Iliev
ref: character:counterpart-001 -> Iona Radu
ref: dialogue-core-subtext:drama -> Mara Iliev and Iona Radu keep circling the truth they cannot safely name yet
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: keep the language intimate, restrained, and emotionally specific
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@suspense-001-core refine
ref: suspense-uncertainty:setup -> Can Mara expose the lie that killed her brother if the truth also proves her family survived on the same silence?
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: keep the language intimate, restrained, and emotionally specific
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@pause-001-core refine
ref: pause-focus:setup -> grief shifts from private mourning into a timed investigation
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: let rhythm changes expose emotional consequence instead of merely changing speed
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@acceleration-001-core refine
ref: acceleration-trigger:setup -> the recording ends with Mara's brother naming Lucian before the tape cuts off
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: let rhythm changes expose emotional consequence instead of merely changing speed
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.
