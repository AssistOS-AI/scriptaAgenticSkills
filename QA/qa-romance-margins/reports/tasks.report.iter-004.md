# Revision tasks

## task-001 — Fix cnl stage: refined plans do not retain placeholders
- Priority: medium
- Stage: cnl
- Action: Repair the cnl stage so the failed check "refined plans do not retain placeholders" passes for qa-romance-margins.
- Evidence: refined plans do not retain placeholders

## task-002 — Increase lexical novelty
- Priority: medium
- Stage: micro
- Action: Diversify scene-level action and trigger phrasing so the originality index rises without weakening continuity.
- Evidence: OI=70

## task-003 — Reinforce character continuity
- Priority: medium
- Stage: cnl
- Action: Carry character beliefs, fears, and relationship changes more explicitly from refined plans into drafts and exports.
- Evidence: CAD_score=88

## task-004 — Improve final prose polish
- Priority: high
- Stage: bookwriter
- Action: Tighten repeated formulations and deepen scene-specific language in the final edition so the narrative quality score moves above the current band.
- Evidence: NQS=88
