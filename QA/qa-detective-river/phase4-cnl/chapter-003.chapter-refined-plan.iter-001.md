...
input-state: In the evidence hangar, Daria and Teo trace the victim's last route through broken camera timestamps.
...
ref: input-state:investigation-2 -> In the evidence hangar, Daria and Teo trace the victim's last route through broken camera timestamps.
ref: output-state:investigation-2 -> administrative negligence hardens into coordinated concealment
ref: conflict:investigation -> Teo wants to publish immediately, while Daria still needs proof that the camera gap conceals homicide rather than corruption alone.
ref: chapter-question:investigation -> Can Daria prove murder before procedure itself is used to wash the evidence clean?
