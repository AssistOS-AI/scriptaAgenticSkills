...
ref: input-state:escalation-1 -> The footage gap lines up exactly with a maintenance override signed by Soren's deputy.
ref: output-state:escalation-1 -> Nadia now knows the cover-up has a clock and a route
...
ref: chapter-question:escalation-1 -> every request for the raw server logs pings Soren before it reaches records
ref: answer-shift:detective-police-escalation-1 -> the cover-up is not separate from the institution; it is how the institution currently functions
ref: world-pressure:escalation-1 -> Theo wants to publish immediately, while Nadia still needs proof that the camera gap conceals homicide rather than corruption alone.
