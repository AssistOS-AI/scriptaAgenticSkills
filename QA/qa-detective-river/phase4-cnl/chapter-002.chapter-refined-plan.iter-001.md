...
ref: input-state:escalation-1 -> In the evidence hangar, Daria and Teo trace the victim's last route through broken camera timestamps.
ref: output-state:escalation-1 -> administrative negligence hardens into coordinated concealment
ref: conflict:escalation -> Teo wants to publish immediately, while Daria still needs proof that the camera gap conceals homicide rather than corruption alone.
ref: chapter-question:escalation -> Can Daria prove murder before procedure itself is used to wash the evidence clean?
