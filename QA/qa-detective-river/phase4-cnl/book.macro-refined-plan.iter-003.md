@central-idea define
hook-pattern: what-if
tension-source: mixed
naming-state: placeholder
hook: Inspector Daria Muntean reopens a river drowning that the county police marked accidental, only to find tide logs, surveillance gaps, and witness transfers pointing back to a redevelopment contract. Diver-journalist Teo Sava helps her track the physical evidence while precinct chief Pavel Orban turns procedure into a moving wall designed to exhaust the case.
protagonist: Daria Muntean
desire: prove that the drowning victim was killed before he entered the river lock
opposition: Pavel Orban, precinct chief tied to the riverfront redevelopment board, backed by County Precinct South
stakes: the victim's true cause of death, the integrity of the precinct, and the city contract tied to the riverfront
dilemma: Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
story-question: Can Daria prove murder before procedure itself is used to wash the evidence clean?
audience-fit: detective / police readers
pitch-test: Daria Muntean must prove that the drowning victim was killed before he entered the river lock while Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
A homicide inspector suspects that a drowning ruled accidental hides a municipal murder and a police cover-up.
The symbolic seed leaves surface naming unresolved on purpose so later refinement can introduce more natural language detail.

@theme define
topic: identity-self
moral-shape: resistance
thematic-question: What remains of justice when institutions become experts at making truth look administrative?
thematic-statement: Procedure stops protecting order the moment it is weaponized against the facts it claims to preserve.
pole-a: order-through-procedure
pole-b: truth-through-breach
moral-pressure: Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
transformation-vector: move Daria Muntean from if she follows procedure closely enough, the truth will eventually surface toward evidence survives only when someone refuses the procedure designed to neutralize it
dramatization: major choices and consequences must carry the theme instead of direct exposition

@wisdom define
wisdom-dominant-dimension: balanced
cognitive: Can Daria prove murder before procedure itself is used to wash the evidence clean?
emotional: accept that lawful procedure can become the crime's most reliable accomplice
moral: Procedure stops protecting order the moment it is weaponized against the facts it claims to preserve.
reflexive: the reflexive dimension of insight this story offers
experiential: the experiential dimension of insight this story offers
perspective-mode: single-perspective
openness: open
anti-didactic-rule: insight must emerge from consequences, reversals, and conflicting voices

@narrative-structure define
information-order: flashback-rich
causal-density: loose
macro-form: linear
beginning: establish Daria Muntean inside the river evidence hangar beside Lock Six and the pressure around County Precinct South
inciting-incident: the body shows wrist abrasions inconsistent with drifting debris
middle: escalation, pressure, and relational strain expose the cost of prove that the drowning victim was killed before he entered the river lock
plot-point-1: Daria Muntean commits to action instead of withdrawal
midpoint: the missing lock-camera drive reveals a hidden layer of the conflict
plot-point-2: Pavel Orban turns pressure into direct threat
climax: Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
resolution: a new equilibrium forms after Daria Muntean accepts irreversible cost
causal-rule: every-escalation-must-narrow-the-safe-options

@narrative-model select
adaptation-strength: faithful
model-name: freytag
core-pattern: desire-opposed-by-system
fit-reason: the detective / police profile needs direct escalation with meaningful internal cost
transformation-logic: Daria Muntean changes while the surrounding system reveals its hidden logic
genre-compatibility: Detective / Police
adaptation-note: the chosen model is bent toward short-form clarity and evidence-rich validation

@blueprint map
premise: Inspector Daria Muntean reopens a river drowning that the county police marked accidental, only to find tide logs, surveillance gaps, and witness transfers pointing back to a redevelopment contract. Diver-journalist Teo Sava helps her track the physical evidence while precinct chief Pavel Orban turns procedure into a moving wall designed to exhaust the case.
exposition: introduce Daria Muntean, the river evidence hangar beside Lock Six, and the first unstable sign around the missing lock-camera drive
rising-action: pressure from Pavel Orban and County Precinct South complicates every attempt to act
midpoint-shift: the missing lock-camera drive changes what the protagonist understands about the conflict
climax: Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
resolution: the core conflict resolves with lasting cost and a redefined relation to County Precinct South
emotional-layer: curiosity -> intrigue -> tension -> mistrust -> shock -> relief
stakes-ladder: private-to-public

@arc-book-main map
arc-axis: identity-self under resistance pressure
opening-state: initial-promise-unstable-but-not-public
escalation-pattern: pressure-converts-hidden-contradiction-into-visible-consequence
midpoint-recognition: apparent-problem-hides-larger-moral-structure
climactic-choice: Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
ending-state: preserves-meaning-by-leaving-cost-visible

@arc-protagonist-main map
entry-belief: if she follows procedure closely enough, the truth will eventually surface control-prevents-loss
core-wound: institutional-loyalty-once-seen-as-justice
pressure-line: every-lawful-move-risks-proving-the-law-compromised
turning-insight: the cover-up is not separate from the institution; it is how the institution currently functions concealment-breeds-larger-collapse
exit-belief: evidence survives only when someone refuses the procedure designed to neutralize it concealment-breeds-larger-collapse

@arc-relationship-main map
pair: Daria Muntean, Teo Sava
entry-dynamic: professional-alliance-with-unequal-risk
stress-pattern: shared-work-keeps-exposing-emotional-imbalance
repair-condition: repair-requires-naming-the-most-damaging-truth
exit-dynamic: scarred-trust-rebuilt-on-painful-clarity

@motif-primary define
motif: watermarked evidence tags
symbolic-function: turn-absence-into-visible-accusation
recurrence-rule: the motif should recur across opening, midpoint, and late consequence scenes with altered meaning

@central-idea refine
ref: hook:what-if -> Inspector Daria Muntean reopens a river drowning that the county police marked accidental, only to find tide logs, surveillance gaps, and witness transfers pointing back to a redevelopment contract. Diver-journalist Teo Sava helps her track the physical evidence while precinct chief Pavel Orban turns procedure into a moving wall designed to exhaust the case.
ref: character:protagonist-001 -> Daria Muntean
ref: desire:protagonist -> prove that the drowning victim was killed before he entered the river lock
ref: opposition:primary -> Pavel Orban, precinct chief tied to the riverfront redevelopment board, backed by County Precinct South
ref: stakes:justice-legitimacy-trust-safety -> the victim's true cause of death, the integrity of the precinct, and the city contract tied to the riverfront
ref: dilemma:central -> Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
ref: story-question:central -> Can Daria prove murder before procedure itself is used to wash the evidence clean?
ref: pitch:what-if -> Daria Muntean must prove that the drowning victim was killed before he entered the river lock while Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: prefer clean procedural detail over melodramatic explanation
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@theme refine
ref: character:protagonist-001 -> Daria Muntean
ref: dilemma:central -> Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
ref: thematic-question:primary -> What remains of justice when institutions become experts at making truth look administrative?
ref: thematic-statement:primary -> Procedure stops protecting order the moment it is weaponized against the facts it claims to preserve.
ref: pole-a:primary -> order-through-procedure
ref: pole-b:primary -> truth-through-breach
ref: entry-belief:protagonist -> if she follows procedure closely enough, the truth will eventually surface
ref: exit-belief:protagonist -> evidence survives only when someone refuses the procedure designed to neutralize it
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: prefer clean procedural detail over melodramatic explanation
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@arc-book-main refine
ref: dilemma:central -> Obey the chain of command and lose the victim forever, or break protocol and prove the station protected the killing.
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make the arc track moral courage as much as clue discovery
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@arc-protagonist-main refine
ref: entry-belief:protagonist -> if she follows procedure closely enough, the truth will eventually surface
ref: exit-belief:protagonist -> evidence survives only when someone refuses the procedure designed to neutralize it
ref: turning-insight:protagonist -> the cover-up is not separate from the institution; it is how the institution currently functions
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make the arc track moral courage as much as clue discovery
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.

@arc-relationship-main refine
ref: character:protagonist-001 -> Daria Muntean
ref: character:counterpart-001 -> Teo Sava
naming-rule: resolved names must be internationally portable proper names and must not embed role words such as inspector, journalist, mayor, or doctor
validation-gate: no placeholder tokens may remain after refinement; if any survive, later drafting and export stages must fail explicitly
should: make the arc track moral courage as much as clue discovery
The refined artifact preserves identifier continuity so deterministic validation can compare seed and successor outputs.
