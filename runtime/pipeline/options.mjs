...
export function normalizePipelineOptions(input = {}) {
  ...
  return {
    bookId,
    title: rawTitle,
    brief: input.brief ?? `symbolic seed for ${baselineProfile} profile`,
    baselineProfile,
    profile,
    constraints: profile.constraints,
    hookPattern: profile.hookPattern,
    tensionSource: profile.tensionSource,
    themeTopic: profile.themeTopic,
    themeShape: profile.themeShape,
    worldDomain: profile.worldDomain,
    magicDeterminacy: profile.magicDeterminacy,
    sequenceType: profile.sequenceType,
    ...
    narratorMode: input.narratorMode ?? profile.narratorMode ?? 'close-third-person',
    focalizationMode: input.focalizationMode ?? profile.focalizationMode ?? 'internal-single',
