...
The current reference runtime fulfills this stage through a deterministic refinement adapter that resolves placeholders from profile lexicons and appends refinement hints. Future remote LLM adapters must preserve the same successor-artifact contract.
