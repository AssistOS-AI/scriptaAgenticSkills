...
The current reference runtime fulfills this stage through a deterministic refinement adapter that resolves placeholders from profile lexicons and appends refinement hints. When names become concrete, they must remain internationally portable proper names rather than locale-locked defaults or role-bearing pseudo-names. Future remote LLM adapters must preserve the same successor-artifact contract.
